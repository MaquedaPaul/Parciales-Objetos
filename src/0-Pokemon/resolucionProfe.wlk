...
class Pokemon {
	...
	method movimientoDisponible() = movimientos.findOrElse({ movimiento => movimiento.estaDisponible() }, { throw new NoPuedeMoverseException(message = "No tiene movimientos disponibles")})
...
class Movimiento {
	...
	method usarEntre(usuario, contrincante) {
		if (!self.estaDisponible()) throw new MovimientoAgotadoException(message = "El movimiento no está disponible")
		usosPendientes -= 1
...
class CondicionEspecial {
	...
	method intentaMoverse(pokemon) {
		if (!self.lograMoverse()) throw new NoPuedeMoverseException(message = "El pokemon no pudo moverse")
	}
...
class Confusion inherits CondicionEspecial {
	...
	override method intentaMoverse(pokemon) {
		...
		try {
			...
			throw new NoPuedeMoverseException(message = "El pokémon no pudo moverse y se hizo daño a sí mismo", cause = e)
		}
	}
