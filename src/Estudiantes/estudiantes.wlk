...
class Estudiante {
	...
	method nivelDeCuelgue(curso) = tareasCompletas.count{ tarea => tarea.curso() == curso } - tareasPendientes.count{ tarea => tarea.curso() == curso }
...
object prudente inherits Selectiva {
	...
	override method elegirTarea(tareas, estudiante) = tareas.max{ tarea => estudiante.tiempoQueRequiere(tarea) }
...
class Prioritaria inherits Selectiva {
	...
	override method elegirTarea(tareas, estudiante) = tareas.findOrElse({ tarea => tarea.materia() == materia }, { tareas.first() })
